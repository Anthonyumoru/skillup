...
// Create tables + seed data - this version won't break
async function initDB() {
  ...
  // Safe lessons table - adds answer column if missing
  await pool.query(`CREATE TABLE IF NOT EXISTS lessons (id SERIAL PRIMARY KEY, skill_id TEXT, title TEXT, content TEXT, question TEXT)`);
  await pool.query(`ALTER TABLE lessons ADD COLUMN IF NOT EXISTS answer TEXT`);
  ...
  // Clear old lessons then insert fresh 30 lessons
  await pool.query(`DELETE FROM lessons`);
  const lessons = [];
  const skills = ['whatsapp','ai','trading','coding','baking','tailor','phone_repair','graphics','social_media','photography'];
  skills.forEach(skill => {
    for(let i=1; i<=3; i++) {
      lessons.push(`('${skill}','Lesson ${i}','Learn step ${i} for ${skill}','What do you learn in step ${i}?','step${i}')`);
    }
  });
  await pool.query(`INSERT INTO lessons (skill_id, title, content, question, answer) VALUES ${lessons.join(',')}`);

  console.log('Database ready + 10 skills + 30 lessons ✅');
}
initDB();
...
app.post('/register', async (req, res) => {
  ...
  await pool.query('INSERT INTO users (id, phone) VALUES ($1, $2) ON CONFLICT (phone) DO UPDATE SET phone=users.phone RETURNING id', [id, phone]);
...
app.post('/lessons', async (req, res) => {
  ...
  const result = await pool.query('SELECT id, skill_id, title, content, question FROM lessons WHERE skill_id = $1', [skill_id]);
  res.json(result.rows);
});
...
app.post('/submit-lesson', async (req, res) => {
  ...
  if(check.rows.length > 0) return res.json({ error: 'Already completed this lesson' });
  ...
  res.json({ success: true, msg: '₦5 added to your balance! ✅' });
});
