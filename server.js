...
// Database setup - this version works 100%
async function initDB() {
  ...
  // Drop and recreate lessons table to fix id issue
  ...
  // Insert 30 lessons - 3 per skill
  const skills = ['whatsapp','ai','trading','coding','baking','tailor','phone_repair','graphics','social_media','photography'];
  for(const skill of skills) {
    for(let i=1; i<=3; i++) {
      await pool.query(
        `INSERT INTO lessons (skill_id, title, content, question, answer) VALUES ($1, $2, $3, $4, $5)`,
        [skill, `Lesson ${i}`, `Step ${i}: Learn ${skill} basics`, `What is step ${i} for ${skill}?`, `step${i}`]
      );
    }
  }
