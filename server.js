...
// Start server FIRST, then init DB. This stops 502 errors
app.listen(PORT, () => {
  console.log(`SkillUp running on port ${PORT}`);
  initDB(); // Run DB setup after server starts
});
...
async function initDB() {
  try {
    ...
    for(const skill of ['whatsapp','ai','trading','coding','baking','tailor','phone_repair','graphics','social_media','photography']) {
      for(let i=1; i<=3; i++) {
        await pool.query(`INSERT INTO lessons (skill_id, title, content, question, answer) VALUES ($1, $2, $3, $4, $5) ON CONFLICT DO NOTHING`,
          [skill, `Lesson ${i}`, `Step ${i}: Learn ${skill} basics`, `What is step ${i} for ${skill}?`, `step${i}`]);
      }
    }
    console.log('Database ready + 10 skills + 30 lessons ✅');
  } catch(e) {
    console.log('DB Error but server still running:', e.message);
  }
}
...
app.post('/submit-lesson', async (req, res) => {
  try {
    ...
    if(check.rows.length > 0) return res.json({ error: 'Already completed' });
    ...
    res.json({ success: true, msg: '₦5 added to balance! ✅' });
  } catch(e) {
    res.json({ error: e.message });
  }
});
...
app.get('/bank-details', (req, res) => {
  res.json({ bank_account: '0224926246', bank_name: 'gtbank' });
});

app.get('/admin', (req, res) => {
  if(req.query.key!== 'mysecret123') return res.send('Access denied');
  res.send('<h1>SkillUp Admin</h1><p>Server is running</p>');
});
