...
// Create tables + seed 10 skills + lessons
async function initDB() {
  ...
  await pool.query(`CREATE TABLE IF NOT EXISTS lessons (id SERIAL PRIMARY KEY, skill_id TEXT, title TEXT, content TEXT, question TEXT, answer TEXT)`);
  ...
  // Seed 3 lessons for each skill = 30 lessons total
  const lessons = [];
  const skills = ['whatsapp','ai','trading','coding','baking','tailor','phone_repair','graphics','social_media','photography'];

  skills.forEach(skill => {
    for(let i=1; i<=3; i++) {
      lessons.push(`('${skill}','Lesson ${i}','This is lesson ${i} content for ${skill}','Question ${i}?','answer${i}')`);
    }
  });

  await pool.query(`INSERT INTO lessons (skill_id, title, content, question, answer) VALUES ${lessons.join(',')} ON CONFLICT DO NOTHING`);
